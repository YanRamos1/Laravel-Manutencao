...
@section('js')
    <script>
        $(document).ready(function () {
            let formNovoTipoEquipamento = $('#formNovoTipo');
            let formNovoEquipamento = $('#formNovoEquipamento');
            formNovoTipoEquipamento.submit(function (e) {
                let nomeType = $('#nomeType').val();
                nomeType = nomeType.toUpperCase();
                if (ValidaNome(nomeType) === false) {
                    alert('Nome do Tipo de Equipamento Inválido');
                    return false;
                } else {
                    nomeType = ValidaNome(nomeType);
                }
                $.ajax({
                    url: '{{route('equipamento_type.create.post')}}',
                    type: 'POST',
                    data: {
                        nomeType: nomeType,
                        _token: '{{csrf_token()}}'
            ...
            formNovoEquipamento.submit(function (e) {
                e.preventDefault();
                let tipoEquipamentoSelect = $('#TipoEquipamentoSelect').val();
                let nomeEquipamento = $('#staticNome').val();
                let serialEquipamento = $('#staticSerial').val();
                let clienteSelect = $('#ClienteSelect').val();

                $.ajax({
                    url: '{{route('equipamento.create.post')}}',
                    type: 'POST',
                    data: {
                        tipoEquipamentoSelect: tipoEquipamentoSelect,
